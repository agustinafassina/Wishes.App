...
export default function HomeClient() {
  ...
  const handleExportManualPDF = async () => {
    ...
    if (!container) {
      toast.error("Error preparing the manual.");
      return;
    }
    setIsExportingManual(true);
    try {
      const res = await fetch("/api/manual");
      if (!res.ok) throw new Error(`Manual not available (${res.status}).`);
      const markdown = await res.text();
      const html = await marked.parse(markdown);
      container.innerHTML = typeof html === "string" ? html : "";
      if (!container.innerHTML.trim()) throw new Error("The manual is empty.");
      ...
      if (scrollHeight < 10 || scrollWidth < 10) {
        throw new Error("Could not render the manual content.");
      }
      ...
      if (canvas.width < 10 || canvas.height < 10) {
        throw new Error("The manual image could not be generated correctly.");
      }
      ...
      pdf.save(fileName);
      toast.success("Manual downloaded as PDF.");
    } catch (error) {
      console.error("Error exporting manual PDF:", error);
      if (manualPdfRef.current) manualPdfRef.current.innerHTML = "";
      toast.error(getApiErrorDisplay(error, "Could not generate the PDF."));
    } finally {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "An error occurred while exporting the PDF. Please try again.";
      toast.error(message);
    } finally {
              ...
              createPortal(
                ...
                >
                  <div className="hamburger-menu hamburger-menu--portal" role="menu" aria-label="App menu">
                    ...
                    <button type="button" className="profile-menu-item" role="menuitem" onClick={() => { setProfileMenuOpen(false); handleExportManualPDF(); }} disabled={isExportingManual}>
                      {isExportingManual ? <span className="profile-menu-spinner" aria-hidden /> : (
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                    ...
                    <button type="button" className="profile-menu-item" role="menuitem" onClick={() => { setProfileMenuOpen(false); handleExportPDF(); }} disabled={isExporting}>
                      {isExporting ? <span className="profile-menu-spinner" aria-hidden /> : (
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                          <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                          <polyline points="7 10 12 15 17 10" />
                          <line x1="12" y1="15" x2="12" y2="3" />
                    ...
                    <button type="button" className="profile-menu-item" role="menuitem" onClick={() => { setProfileMenuOpen(false); handleBackup("json"); }} disabled={isExportingBackup}>
                      {isExportingBackup ? <span className="profile-menu-spinner" aria-hidden /> : (
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                          <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                          <polyline points="7 10 12 15 17 10" />
                          <line x1="12" y1="15" x2="12" y2="3" />
                    ...
                    <button type="button" className="profile-menu-item" role="menuitem" onClick={() => { setProfileMenuOpen(false); handleBackup("csv"); }} disabled={isExportingBackup}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                        <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                        <polyline points="7 10 12 15 17 10" />
                        <line x1="12" y1="15" x2="12" y2="3" />
